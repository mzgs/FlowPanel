...
func TestBuildConfigValidatesStaticAndAppDomains(t *testing.T) {
	...
	cfg, summary, err := buildConfig(":8080", ":9080", ":9443", ":32109", []domain.Record{
		{
	...
	}, nil, nil, nil, runtimeSyncModeStandard)
	if err != nil {
...
func TestBuildConfigIncludesPanelRoute(t *testing.T) {
	cfg, summary, err := buildConfig(
		":18080",
		":9080",
		":9443",
	...
	if !bytes.Contains(rawConfig, []byte(`"panel.example.com"`)) {
		t.Fatalf("raw config = %s, want panel hostname", string(rawConfig))
	}
	if !bytes.Contains(rawConfig, []byte(`"dial":"localhost:18080"`)) {
		t.Fatalf("raw config = %s, want panel upstream", string(rawConfig))
	}
	...
	if httpApp.Servers["public"] == nil {
		t.Fatal("expected public server for panel route")
	}
	if len(httpApp.Servers["public"].Routes) != 1 {
		t.Fatalf("route count = %d, want 1", len(httpApp.Servers["public"].Routes))
	}
}

func TestBuildConfigRejectsReverseProxyTargetsWithPaths(t *testing.T) {
	_, _, err := buildConfig(":8080", ":9080", ":9443", ":32109", []domain.Record{
		{
...
func TestBuildConfigHTTPSOnlyModeDisablesRedirectsAndHTTPChallenge(t *testing.T) {
	...
	}, nil, nil, nil, runtimeSyncModeHTTPSOnly)
	if err != nil {
		t.Fatalf("build config: %v", err)
	}
...
func TestRuntimeSyncCanReloadMultipleDomainSets(t *testing.T) {
	...
	runtime := NewRuntime(zap.NewNop(), ":18080", httpAddr, httpsAddr, fakePHPManager{}, fakePHPMyAdminManager{}, ":32109")
