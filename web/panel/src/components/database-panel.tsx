...
export function DatabasePanel({
  ...
  const scopedDatabases = scopedDomain
    ? databases.filter((database) => database.domain === scopedDomain)
    : databases;
  const filteredDatabases = scopedDatabases.filter((database) => {
    if (!normalizedSearch) {
      return true;
    }
  ...
  return (
    <>
      ...
      <div className={embedded ? "space-y-4" : "px-4 py-6 sm:px-6 lg:px-8"}>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <select
                id="database-domain"
                value={form.domain}
                disabled={dialogMode === "create" && scopedDomain !== ""}
                onChange={(event) => {
                ...
                className={cn(
                  ...
                  dialogMode === "create" && scopedDomain !== ""
