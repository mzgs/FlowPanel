...
import { segmentedTabClassName, segmentedTabListClassName } from "@/components/ui/tabs";
  ...
  return (
    <section className="rounded-xl border border-[var(--app-border)] bg-[var(--app-bg-2)] px-4 py-4 shadow-[var(--app-shadow)]">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div role="tablist" aria-label="System monitor ranges" className={segmentedTabListClassName}>
          {monitorRanges.map((item) => {
            ...
            return (
              <button
                ...
                tabIndex={active ? 0 : -1}
                data-active={active}
                className={cn(segmentedTabClassName, "h-7 px-2.5 text-[11px]")}
                onClick={() => {
                  setRange(item.value);
                }}
              >
